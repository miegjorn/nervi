...
//! `occitan.ops.sre.alerts`, and an independent developer consumer fetches it back with
...
const ALERTS_SUBJECT: &str = "occitan.ops.sre.alerts";

/// A second `occitan.*` subject used to prove subject filtering: a consumer of
/// `occitan.ops.sre.alerts` must not receive traffic published elsewhere on the stream.
const NOISE_SUBJECT: &str = "occitan.ops.metrics.cpu";
...
/// locally it may carry state from a prior run). Deletes the OCCITAN stream if it
/// exists — `ensure_stream` then recreates it clean.
async fn reset_occitan_stream(url: &str) {
    ...
    let _ = js.delete_stream("OCCITAN").await;
}

/// Full chain: SRE producer → `occitan.ops.sre.alerts` → developer consumer, with
...
async fn sre_alert_reaches_developer_consumer_intact_after_late_subscription() {
    let url = nats_url();
    reset_occitan_stream(&url).await;

    let client = NerviClient::connect(&url)
        .await
        .expect("connect to NATS JetStream — is a `nats-server -js` reachable?");

    // The OCCITAN stream must exist before anything is published. nervi-core owns
    ...
    let stream = client
        ...
        .await
        .expect("provision the OCCITAN stream");
    assert_eq!(stream, "OCCITAN", "subject root maps to the OCCITAN stream");
    ...
    // Noise on a *different* occitan subject — the developer consumer must not see it.
    client
        .publish(PublishOptions {
    ...
    let received = client
        ...
        .await
        .expect("subscribe to occitan.ops.sre.alerts");

    // Subject filtering: exactly the one alert, none of the noise.
    assert_eq!(
        received.len(),
        1,
        "developer consumer receives only occitan.ops.sre.alerts traffic, got: {received:?}"
    );
